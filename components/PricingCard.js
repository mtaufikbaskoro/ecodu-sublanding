...
const PricingCard = (props) => {
    ...
    useEffect(() => {
        if(page === 'stan'){
            ...
            setClassNames({
                ul: 'list-disc space-y-2.5 pl-6 max-h-72 overflow-y-auto scrollbar-thin scrollbar-thumb-utbk-200',
                div: 'flex flex-col justify-between gap-5 p-6 bg-utbk-100 text-additional-color-200 font-semibold rounded-lg drop-shadow-2xl',
                a: 'text-center bg-additional-color-300 hover:bg-additional-color-200 text-utbk-200 uppercase py-3.5 rounded-lg font-bold cursor-pointer transition ease-in-out'
            })
        }
    }, [])
    return(
